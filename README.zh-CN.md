...
| 配置项 | 说明 |
|---|---|
| `启用插件` | 全局启用或禁用 SubZ。默认：`true`。 |
| `仅手动目标模式` | 跳过自动入库触发，仅允许手动目标执行。默认：`false`。 |
| `目标语言` | 翻译输出目标语言代码。默认：`zh-CN`。若已存在该语言字幕会自动跳过。 |

#### 手动执行

| 配置项 | 说明 |
|---|---|
| `手动执行目标文件夹` | 手动任务的文件夹路径。默认：空。会递归扫描子目录。 |
| `手动执行目标文件` | 手动任务的单文件路径。默认：空。与文件夹二选一。 |
| `立即执行一次` | 保存时受理并入队一次手动任务。默认：`false`。受理后会自动复位。 |

#### 输出

| 配置项 | 说明 |
|---|---|
| `输出格式` | `srt` 或 `ass`。默认：`srt`。 |
| `ASS 字体名称` | ASS 字体族名称。默认：`微软雅黑`。需在渲染环境可用。 |
| `ASS 字体大小` | ASS 字号。默认：`60`。 |
| `ASS 字体颜色` | ASS 主文字颜色。默认：`&H00FFFFFF`。支持 `&H00RRGGBB` 或 `#RRGGBB`。 |

#### 模型接口

| 配置项 | 说明 |
|---|---|
| `API 提供方` | 提供方标识。默认：`deepseek`。 |
| `API 地址` | LLM 接口基础地址（调用 `/chat/completions`）。默认：`https://api.deepseek.com`。 |
| `API 密钥` | LLM 鉴权密钥。默认：空。 |
| `模型` | 请求中的模型标识。默认：`deepseek-v4-flash`。 |
| `每批字幕条数` | 单次请求包含字幕条数。默认：`120`。值越大吞吐越高，但超时/错位风险也更高。 |
| `源语言偏好` | 外挂字幕优先匹配语言代码。默认：`en`。未命中时回退到任意可用字幕。 |

#### 媒体工具

| 配置项 | 说明 |
|---|---|
| `FFmpeg 路径` | 抽取封装字幕用的可执行路径。默认：`/bin/ffmpeg`。 |
| `FFprobe 路径` | 探测字幕轨信息用的可执行路径。默认：`/bin/ffprobe`。 |

#### 日志与状态

| 配置项 | 说明 |
|---|---|
| `日志文件最大大小(MB)` | 单个运行日志滚动上限。默认：`10`。 |
| `日志保留天数` | 运行日志保留天数。默认：`7`。 |
| `详细日志模式(Debug)` | 记录完整请求/响应正文及 ffprobe/ffmpeg 细节。默认：`false`。 |
| `状态页访问路径` | 外部状态页链接保存字段。默认：`http://localhost:18123/subz-status.html`。 |

#### 稳定性

| 配置项 | 说明 |
|---|---|
| `保留字幕标签` | 翻译前后保护并恢复标签/占位符。默认：`true`。 |
| `启用尾段重试` | 对失败或未变化片段执行自愈重试。默认：`true`。 |
| `尾段重试次数` | 自愈重试最大轮数。默认：`2`。 |
| `优先非强制字幕轨` | 选源时优先非强制字幕轨。默认：`true`。 |
| `优先非听障字幕轨` | 选源时优先非听障字幕轨。默认：`true`。 |
| `优先文本字幕轨` | 选源时优先文本字幕轨。默认：`true`。 |
